...
class SampleGenerator(nn.Module):
    ...
    def __init__(self, decoder, batch_size=1, cond_repeat=800, wav_freq=16000):
        super().__init__()
        self.decoder = decoder
        self.decoder.shift_input = False
        self.cond_repeat = cond_repeat
        self.wav_freq = wav_freq
        self.batch_size = batch_size
        self.was_cuda = next(self.decoder.parameters()).is_cuda

        x = torch.zeros(self.batch_size, 1, 1)
        x = x.cuda() if self.was_cuda else x
        self.decoder.first_conv = QueuedConv1d(self.decoder.first_conv, x)

        x = torch.zeros(self.batch_size, self.decoder.residual_channels, 1)
        x = x.cuda() if self.was_cuda else x
    ...
    def init(self, batch_size=None):
        ...
        x = torch.zeros(self.batch_size, 1, 1)
        x = x.cuda() if self.was_cuda else x
        self.decoder.first_conv.init_queue(x)
